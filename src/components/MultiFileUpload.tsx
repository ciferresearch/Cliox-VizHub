...
const MultiFileUpload: React.FC<MultiFileUploadProps> = ({ onFilesProcessed }) => {
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      <h3 className="text-xl font-medium mb-6">Upload Your Files</h3>
      ...
      <ul className="mb-4 space-y-1.5">
        {fileNamingRules.map((rule, index) => (
          <li key={index} className="flex items-baseline">
            ...
            <span className="text-sm text-gray-500 ml-2">- {rule.description}</span>
          </li>
        ))}
      </ul>
      ...
      {uploadMode === 'zip' ? (
          ...
          <div className="relative">
            ...
            >
              <div className="text-center">
                <svg className={`h-12 w-12 mx-auto mb-3 ${zipFile ? 'text-blue-500' : 'text-gray-400'}`} 
                  fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} 
                    d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
                </svg>
                <span className={`block text-sm font-medium ${zipFile ? 'text-blue-600' : 'text-gray-700'}`}>
        ...
        <div className="">
          <div className="relative">
            ...
            >
              <div className="text-center">
                <svg className={`h-12 w-12 mx-auto mb-3 ${files.length > 0 ? 'text-blue-500' : 'text-gray-400'}`} 
                  fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} 
                    d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <span className={`block text-sm font-medium ${files.length > 0 ? 'text-blue-600' : 'text-gray-700'}`}>
              ...
              <div className="max-h-40 overflow-y-auto bg-gray-50 rounded-lg p-3">
                <ul className="text-xs text-gray-600 space-y-1.5">
                  {files.map((file, index) => {
                    const visualizationType = mapFileToVisualizationType(file.name);
                    const typeColor = visualizationType ? 'text-green-600' : 'text-amber-600';
                    const fileTypeText = visualizationType 
                      ? `(${visualizationType})` 
                      : '(unknown format)';
                    
                    return (
                      <li key={index} className="flex items-center p-1 border-b border-gray-100 last:border-b-0">
                        <svg className="h-3 w-3 mr-1.5 text-blue-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
