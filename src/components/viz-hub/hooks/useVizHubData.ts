 ...
 */
export function useVizHubData(externalData?: VizHubData, config?: VizHubConfig) {
  const {
    checkDataStatus,
    dataStatus,
    processingStatus,
  ...
  useEffect(() => {
    ...
    const dataChanged = JSON.stringify(externalData) !== JSON.stringify(lastExternalDataRef.current);
    ...
    if (!hasInjectedData.current) {
      ['wordCloudData', 'dateDistributionData', 'emailDistributionData', 'sentimentData', 'documentSummaryData'].forEach(key => {
    ...
    if (externalData.emailDistribution) {
      const csvData = convertToCSV(externalData.emailDistribution, ['emails_per_day']);
    ...
    if (externalData.dateDistribution) {
      const csvData = convertToCSV(externalData.dateDistribution, ['time', 'count']);
    ...
    if (externalData.sentiment) {
      localStorage.setItem('sentimentData', JSON.stringify(externalData.sentiment));
    ...
    if (externalData.wordCloud) {
      localStorage.setItem('wordCloudData', JSON.stringify(externalData.wordCloud));
    ...
    if (externalData.documentSummary) {
      localStorage.setItem('documentSummaryData', JSON.stringify(externalData.documentSummary));
  ...
  return {
    ...
    dataSourceInfo: {
      ...
      usingLocalStorage: !externalData && Object.values(dataStatus).some(status => status)
    }
  };
} 